...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        qos_latched = QoSProfile(depth=10, durability=QoSDurabilityPolicy.TRANSIENT_LOCAL)
        qos_clock = QoSProfile(depth=1)
        # publishers
        self.cmd_vel_pub = self.create_publisher(Twist, self.velo_topic, qos)
        # subscribers
        self.goal_pose_sub = self.create_subscription(Pose, self.goal_topic, self.goal_pose_callback, qos_latched)
