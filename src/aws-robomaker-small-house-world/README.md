...
# Load directly into Gazebo Harmonic (without ROS)
```bash
export GZ_SIM_RESOURCE_PATH=`pwd`:`pwd`/models:`pwd`/worlds
gz sim -r worlds/small_house.world
```

# ROS Launch with Gazebo Harmonic viewer (without a robot)
```bash
# build for ROS
